...
Compiler.prototype.compileNode = function(node, jsx) {
	...
	Walk.recursive(node, null, {
		JSXElement: function(node, _state, _walk) {
			...
			if (isElement) js.push(self.factory + "(" + renderElementName(name))
			else if (self.componentFactory) js.push(
				(self.componentFactory[0] == "." ? self.factory : "") +
				self.componentFactory + "(" + renderVariableName(name)
			)
			else js.push(renderVariableName(name) + "(")
		...
		JSXFragment: function(node, _state, _walk) {
			...
			var js = self.fragmentFactory
				? (self.fragmentFactory[0] == "." ? self.factory : "") +
					self.fragmentFactory +
					"(null, " + children + ")"
				: children
...
Compiler.prototype.compileAttributes = function(attrs, jsx) {
	...
	if (groups.length > 1) {
		js.push((this.assign[0] == "." ? this.factory : "") + this.assign + "(")
...
function applyReplacements(replacee, replacements) {
	...
	replacements.forEach(function(replacement) {
		...
		if (last < start) output.push(replacee.slice(last, start))
		else if (start < last) throw new RangeError("Operlapping replacements")
	...
	if (last < replacee.length - 1) output.push(replacee.slice(last))
	return output.join("")
}
