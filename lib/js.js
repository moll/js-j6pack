...
function Compiler(opts) {
	if (opts) {
		...
		if (opts.passUnknownTagsToFactory != null)
			this.passUnknownTagsToFactory = opts.passUnknownTagsToFactory
	}
}

Compiler.prototype.factory = "Jsx"
Compiler.prototype.assign = "Object.assign"
Compiler.prototype.passUnknownTagsToFactory = false
...
Compiler.prototype.compileNode = function(node, jsx) {
	...
	Walk.recursive(node, null, {
		JSXElement: function(node, _state, _walk) {
			...
			else if (self.passUnknownTagsToFactory)
				js.push(self.factory + "(" + renderVariableName(name))
			else
				js.push(renderVariableName(name) + "(")

			if (attrs.length > 0) {
				if (isElement || self.passUnknownTagsToFactory) js.push(",")
				...
				js.push(
					!isElement && !self.passUnknownTagsToFactory || /^\n/.test(attrsJs)
					? attrsJs : " " + attrsJs
				)
			}
			else if (children.length > 0) {
				if (isElement || self.passUnknownTagsToFactory) js.push(", null")
				else js.push("null")
			}
		...
		JSXFragment: function(node, _state, _walk) {
			...
			replacements.push({
				...
				text: "[" + self.compileChildren(node.children, jsx) + "]"
			})
		},
...
OffsetText.prototype.splice = function(start, end, replacement) {
	return new OffsetText(splice(
		this.text,
		Math.max(start - this.offset, 0),
		end == null ? undefined : Math.max(end - this.offset, 0),
		replacement
	), this.offset)
}
